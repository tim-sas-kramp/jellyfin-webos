...
function navigate(direction, scope) {
    var currentScope = scope || getNavigationScope();
    var allElements = getFocusableElements(currentScope);
    ...
    if (!element || !scopeContains(currentScope, element) || !isVisible(element) || element.tagName == 'BODY') {
        allElements[0].focus();
        return;
    }

    if (currentScope && currentScope.id === 'userPicker') {
...
function moveWhoWatchingHorizontal(amount) {
    ...
    if (isPinPromptVisible()) {
        navigate(amount < 0 ? 'left' : 'right', getPinPromptScope());
        return;
    }
...
function moveWhoWatchingDown() {
    ...
    if (isPinPromptVisible()) {
        ...
        navigate('down', getPinPromptScope());
        return;
    }
...
document.onkeydown = function (evt) {
    ...
    if (!isLauncherNavigationActive() && !isWhoWatchingVisible()) {
        return;
    }
...
function navigationInit() {
    if (isWhoWatchingVisible() && whoWatching && whoWatching.focusFirst) {
        whoWatching.focusFirst();
        return;
    }
...
function handoff(url, bundle, options) {
    ...
    if (options.serverInfo) {
        currentHandoffContext.server.baseurl = getServerAddress(options.serverInfo) || currentHandoffContext.server.baseurl;
        currentHandoffContext.server.hosturl = currentHandoffContext.server.hosturl || url;
        currentHandoffContext.server.serverId = getServerId(options.serverInfo) || currentHandoffContext.server.serverId;
        currentHandoffContext.server.serverName = getServerName(options.serverInfo) || currentHandoffContext.server.serverName;
    }

    if (options.serverCredential) {
        currentHandoffContext.jellyfinCredentials = AccountAuth.mergeJellyfinWebCredential(
            currentHandoffContext.jellyfinCredentials,
            options.serverCredential
        );

        currentHandoffContext.server.baseurl = options.serverCredential.ManualAddress || currentHandoffContext.server.baseurl;
        currentHandoffContext.server.serverId = options.serverCredential.Id || currentHandoffContext.server.serverId;
        currentHandoffContext.server.serverName = options.serverCredential.Name || currentHandoffContext.server.serverName;
    }
...
function handleEmbeddedSignIn(data) {
    ...
    if ((pendingAuthContext || activeAccountContext) && data.accessToken) {
        handleLocalUserSignedIn({
            user: {
                Id: data.userId,
                Name: data.displayName || data.userId
            },
            accessToken: data.accessToken
        });
    }
...
function handleEmbeddedSignOut(logoutInfo) {
    ...
    if (pendingAuthContext || activeAccountContext) {
        handleLocalUserSignedOut(logoutInfo && logoutInfo.logoutInfo ? logoutInfo.logoutInfo : logoutInfo);
    }
...
window.addEventListener('message', function (event) {
    ...
    switch (msg.type) {
        ...
        case 'multiUser.signedOut':
            handleEmbeddedSignOut(msg.data || {});
            break;
        case 'AppHost.exit':
            webOS.platformBack();
            break;
